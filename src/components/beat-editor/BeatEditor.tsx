...
export default function BeatEditor({ 
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-[#1a1a24] border border-gray-200 dark:border-white/5 rounded-2xl overflow-hidden">
      {/* Header */}
            ...
            <button
              ...
              className={`px-3 py-2 text-sm rounded-lg transition-colors ${
                selectedTemplate === template.name
                  ...
                  : 'bg-gray-100 dark:bg-white/10 text-gray-800 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20 border border-gray-200 dark:border-white/10'
              }`}
            >
              {template.name}
            </button>
          ...
          <AnimatePresence>
            {beats.map((beat) => (
              <motion.div
                ...
                className={`group relative p-4 rounded-lg border-2 transition-all bg-gray-50/50 dark:bg-white/5 ${
                  draggedBeat === beat.id
                    ? 'border-purple-500 opacity-50'
                    : 'border-gray-200 dark:border-white/10 hover:border-purple-500/50 dark:hover:border-purple-500/50'
                }`}
              >
                <div className="flex items-start gap-3">
                  <GripVertical className="w-5 h-5 text-gray-400 dark:text-gray-500 mt-1 cursor-grab" />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <input
                        ...
                        className="flex-1 bg-transparent text-gray-800 dark:text-gray-200 font-medium focus:outline-none"
                      />
                    </div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-gray-100 dark:bg-white/10 text-gray-800 dark:text-gray-300 text-sm rounded-lg p-2 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 border border-gray-200 dark:border-white/10"
                      rows={2}
                    />
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <span className="text-xs text-gray-500 dark:text-gray-400">{beat.duration}%</span>
                      </div>
                      <div>
                        <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1">Impact</label>
                        <input
                          type="range"
                          min={1}
                          max={10}
                          ...
                          className="w-full"
                        />
                        <span className="text-xs text-gray-500 dark:text-gray-400">{beat.emotionalImpact}/10</span>
                      </div>
                    </div>
                  </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-white/5 space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isAnalyzing ? (
            <>
        ...
        {analysis && (
          <motion.div
            ...
            className="bg-gray-100 dark:bg-white/10 rounded-lg p-3 space-y-2 border border-gray-200 dark:border-white/10"
          >
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">Balance Score</span>
              <span className="text-lg font-bold text-gray-800 dark:text-gray-200">{analysis.balanceScore}/100</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">Pacing</span>
              <span className={`text-sm font-medium ${
                analysis.pacing === 'balanced' ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'
              }`}>
                {analysis.pacing}
              </span>
            </div>
